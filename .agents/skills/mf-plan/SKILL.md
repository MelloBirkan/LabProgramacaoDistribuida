...
- Codex: keep using `AskUserQuestion`.
...
Keep each round focused. Codex references allow up to 4 questions per round; Codex `request_user_input` supports up to 3. Multiple clarification rounds are expected.

## Tool equivalents

The reference files were converted from Codex commands and may mention Codex tool names. Apply these equivalents in Codex:

| Codex command instruction | Codex equivalent |
| --- | --- |
